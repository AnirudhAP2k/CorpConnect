...
import type { EmbedEventPayload, EmbedOrgPayload } from "@/lib/types";
...
async function postToAiService(
	...
	try {
		...
		console.error(
			`[Embed] Error generating ${service} embedding for service ${service} and id ${body.eventId || body.orgId}:`,
			error,
		);
		throw error;
	}
...
export async function processEmbedEvent(payload: EmbedEventPayload) {
	...
	console.log(
		`[Embed] Generating event embedding for "${event.title}" (${eventId})`,
	);
