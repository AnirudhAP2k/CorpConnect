...
import type { ConnectionNotificationPayload } from "@/lib/types";
...
export async function processConnectionNotification(
	payload: ConnectionNotificationPayload,
) {
	...
	if (!connection) {
		console.warn(
			`[Job] Connection ${connectionId} not found, skipping notification`,
		);
		return;
	}
	...
	const eventMap: Record<
		string,
		{ emailEvent: ConnectionEmailEvent; notifyOrgId: string }
	> = {
		CONNECTION_REQUEST: {
			emailEvent: "REQUESTED",
			notifyOrgId: connection.targetOrgId,
		},
		CONNECTION_ACCEPTED: {
			emailEvent: "ACCEPTED",
			notifyOrgId: connection.sourceOrgId,
		},
		CONNECTION_DECLINED: {
			emailEvent: "DECLINED",
			notifyOrgId: connection.sourceOrgId,
	...
	const notifyOrg =
		meta.notifyOrgId === connection.sourceOrgId
	...
	const recipients: { id: string; name: string; email: string }[] = (
		notifyOrg.members as OrganizationMemberWithUser[]
	)
		.map((m: OrganizationMemberWithUser) => ({
			id: m.user.id,
	...
	if (recipients.length === 0) {
		console.warn(
			`[Job] No admin emails found for org ${notifyOrg.id}, skipping`,
		);
		return;
	}

	console.log(
		`[Job] Sending ${meta.emailEvent} notification to ${recipients.length} admin(s) of ${notifyOrg.name}`,
	);
	...
	console.log(
		`[Job] ✓ Connection ${meta.emailEvent} notifications sent for connection ${connectionId}`,
	);
}
