...
export async function processMeetingNotification(
	...
	const emailEventMap: Record<
		string,
		{ emailEvent: MeetingEmailEvent; notifyOrg: any; actorOrg: any }
	> = {
		MEETING_REQUEST: {
			emailEvent: "REQUESTED",
			notifyOrg: mr.receiverOrg,
			actorOrg: mr.senderOrg,
		},
		MEETING_ACCEPTED: {
			emailEvent: "ACCEPTED",
			notifyOrg: mr.senderOrg,
			actorOrg: mr.receiverOrg,
		},
		MEETING_DECLINED: {
			emailEvent: "DECLINED",
			notifyOrg: mr.senderOrg,
	...
	if (!meta) {
		console.warn(`[Job] Unknown meeting notif type: ${type}`);
		return;
	}
	...
	if (recipients.length === 0) {
		console.warn(`[Job] No admin emails for org ${meta.notifyOrg.id}`);
		return;
	}
	...
	console.log(
		`[Job] ✓ ${meta.emailEvent} meeting notification sent for request ${meetingRequestId}`,
	);
}
