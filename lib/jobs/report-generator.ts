...
import { GenerateReportPayload } from "@/lib/types";

// ─── Main handler ─────────────────────────────────────────────────────────────

export async function processEventReport(
	...
	if (org.subscriptionPlan !== "ENTERPRISE") {
		console.log(
			`[report-generator] ⏭ Org ${org.id} is not ENTERPRISE — skipping report.`,
		);
		return;
	}
	...
	if (existing) {
		console.log(
			`[report-generator] ⏭ Report for event ${eventId} already exists — skipping.`,
		);
		return;
	}
	...
	const avgRating =
		ratedFeedbacks.length > 0
			? ratedFeedbacks.reduce((s, f) => s + (f.rating ?? 0), 0) /
	...
	const avgSentimentScore =
		scoredFeedbacks.length > 0
			? scoredFeedbacks.reduce((s, f) => s + (f.sentimentScore ?? 0), 0) /
	...
	let aiExecutiveSummary =
		"AI executive summary not available. Please review metrics above.";
	let overallScore = 0;
	...
	if (summaryResult) {
		...
		if (summaryResult.strengths.length > 0) {
			console.log(
				`[report-generator] ✓ AI summary generated — score: ${overallScore}`,
			);
		}
	} else {
		console.warn(
			`[report-generator] ⚠ AI service unavailable — using placeholder summary.`,
		);
	}
	...
	const avgDurationMins =
		avgDurationSecs !== null ? Math.round(avgDurationSecs / 60) : null;
	...
	console.log(
		`[report-generator] ✓ Report emails sent to ${recipients.length} recipients.`,
	);
}
