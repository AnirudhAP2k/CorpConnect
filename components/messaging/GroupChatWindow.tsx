...
export function GroupChatWindow({
	...
	return (
		<div className="flex flex-col h-full relative bg-nx-surface text-nx-on-surface">
			{/* ── Chat Header ── */}
			<div className="shrink-0 flex items-center justify-between gap-3 px-6 py-4 border-b border-nx-outline-variant bg-nx-surface-container-lowest">
				...
				<div className="flex items-center gap-2 shrink-0">
					{!connected && (
						<div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-nx-warning-container border border-nx-warning/30 text-nx-on-warning-container text-xs font-medium">
			...
			<div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-1.5">
				...
				{hasMore && (
					<button
						...
						disabled={isLoading}
						className="self-center flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-nx-outline-variant bg-nx-surface-container-lowest text-xs text-nx-on-surface-variant hover:bg-nx-surface-container transition-colors mb-2 disabled:opacity-50"
					>
				...
				{messages.length === 0 && !isLoading && (
					<div className="flex-1 flex flex-col items-center justify-center text-center py-16">
						...
						<p className="text-sm font-semibold text-nx-on-surface">
							No messages yet
						</p>
			...
			<MessageInput
				...
				disabled={!connected}
				placeholder={
					connected ? `Message ${groupName}…` : "Reconnecting to server…"
				}
			/>
