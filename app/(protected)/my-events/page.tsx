...
import { getUserParticipations, getUserHostedEvents } from "@/data/event-participation";
import Image from "next/image";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import {
    Calendar, MapPin, Building2, Clock, CheckCircle2,
    XCircle, Ticket, Megaphone, Users, Pencil,
} from "lucide-react";
import { format } from "date-fns";
import Link from "next/link";
import CancelParticipationButton from "@/components/shared/CancelParticipationButton";

type Tab = "upcoming" | "past" | "hosted" | "cancelled";
...
export default async function MyEventsPage({ searchParams }: MyEventsPageProps) {
    ...
    // Fetch participations and hosted events in parallel
    const [allParticipations, hostedEvents] = await Promise.all([
        getUserParticipations(session.user.id),
        getUserHostedEvents(session.user.id),
    ]);
    ...
    return (
        <div className="wrapper py-8">
            ...
            {/* Hosted Events Tab */}
            {activeTab === "hosted" ? (
                <HostedEventsList events={hostedEvents} now={now} />
            ) : (
                /* Participation Tabs (Upcoming / Past / Cancelled) */
                <ParticipationList
...
type HostedEvent = Awaited<ReturnType<typeof getUserHostedEvents>>[number];

function HostedEventsList({ events, now }: { events: HostedEvent[]; now: Date }) {
    if (events.length === 0) {
        return (
            <div className="text-center py-16">
                ...
                <p className="text-gray-500 mb-6">
                    You haven&apos;t hosted any events yet. Create one through your organization.
                </p>
            </div>
        );
    }
        ...
        <div className="space-y-4">
            {events.map((event) => {
                ...
                const badgeConfig = isLive
                    ? { label: "Live", className: "bg-red-100 text-red-700" }
                    : isUpcoming
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <Badge className={`${badgeConfig.className} flex items-center gap-1 flex-shrink-0`}>
                                    {isLive && (
                            ...
                            <div className="flex flex-wrap gap-4 mt-3 text-sm text-gray-500">
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span>
                                        {event._count.participations} attendee{event._count.participations !== 1 ? "s" : ""}
...
/* ─── Participation List (Upcoming / Past / Cancelled) ───────────────────────── */

type Participation = Awaited<ReturnType<typeof getUserParticipations>>[number];
